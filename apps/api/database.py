        ...
        try:
            ...
            if row:
                try:
                    ...
                    return {"timestamp": row["timestamp"], "data": row["payload"]}
            return None
        except sqlite3.Error as e:
    ...
    try:
        ...
        for row in rows:
            try:
                ...
                payload = row["payload"]
            result.append({"timestamp": row["timestamp"], "data": payload})
        return result
    except sqlite3.Error as e:
...
def get_sensor_summary(device_id: str, hours: int = 24):
    ...
    try:
        cursor = conn.cursor()
        # Sentinel: Use parameterized query and concatenation for time interval to prevent SQL injection
        cursor.execute(
            """
            SELECT
...
def get_energy_analytics(device_id: str, days: int = 1):
    ...
    try:
        ...
        # Sentinel: Use parameterized query for time interval to prevent SQL injection
        cursor.execute(
            """
            SELECT 
