...
def get_sensor_summary(device_id: str, hours: int = 24):
    ...
    try:
        cursor = conn.cursor()
        # Security: Using parameterized query with manual concatenation for SQLite datetime
        # to prevent SQL injection via the 'hours' variable.
        cursor.execute('''
            SELECT
...
def get_energy_analytics(device_id: str, days: int = 1):
    ...
    try:
        ...
        # Security: Parameterized 'days' to prevent SQL injection.
        cursor.execute('''
            SELECT 
