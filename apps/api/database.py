...
def get_sensor_summary(device_id: str, hours: int = 24):
    ...
    try:
        cursor = conn.cursor()
        # Security: Use parameterized query to prevent SQL injection via hours parameter.
        cursor.execute('''
            SELECT
...
def get_energy_analytics(device_id: str, days: int = 1):
    ...
    try:
        ...
        # Security: Use parameterized query to prevent SQL injection via days parameter.
        cursor.execute('''
            SELECT 
