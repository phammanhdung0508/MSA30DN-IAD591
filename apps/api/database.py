...
def get_device_data_history(device_id: str, limit: int = 100):
    # Defense-in-depth: clamp limit to safe range
...
def get_sensor_summary(device_id: str, hours: int = 24):
    # Defense-in-depth: clamp hours to safe range
    ...
    try:
        cursor = conn.cursor()
        # Use parameterized query and string concatenation for SQLite interval
        cursor.execute('''
            SELECT
...
def get_energy_analytics(device_id: str, days: int = 1):
    ...
    """
    # Defense-in-depth: clamp days to safe range
