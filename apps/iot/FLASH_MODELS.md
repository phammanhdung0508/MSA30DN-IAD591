...
Tài liệu này tạo quy trình chuẩn để build firmware và nạp model Wake Word (hiện tại: **"Hi, Jason"**).

## 1. Cấu hình model (đã bật sẵn trong repo)

Repo đã bật WakeNet9 (**Hi, Jason**) và VADNET1 medium trong:
...
- `ESP Speech Recognition -> Load Multiple Wake Words (WakeNet9) -> Hi,Jason (wn9_hijason_tts2)`
...
Ví dụ offset thực tế của dự án hiện tại thường là `0x005f0000` (có thể thay đổi nếu bạn chỉnh `partitions.csv`).
...
Ví dụ (offset `0x005f0000`):

```bat
%USERPROFILE%\.platformio\packages\tool-esptoolpy\esptool.py --chip esp32s3 --port COMx write_flash 0x005f0000 srmodels\srmodels.bin
```
...
- Luồng audio hiện dùng **TCP**. Nếu cần kiểm thử, xem `apps/api/audio_tcp.py` và port `AUDIO_TCP_PORT` (mặc định 3334).
