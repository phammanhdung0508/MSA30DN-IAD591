...
export function SensorsView() {
    ...
    useEffect(() => {
        const fetchSummary = async () => {
            const [sum, hist] = await Promise.all([getSensorSummary(), getSensorHistory("esp32-main", 10)]);
    ...
    const historyItems = useMemo(() => history?.items?.slice(0, 6) ?? [], [history]);
        ...
        <div className="flex h-full flex-col">
            <header className="px-6 pt-6 pb-4">
                <div>
                    ...
                    <div className="text-2xl font-semibold text-white">DHT11 + MQ-135</div>
                </div>
            </header>
            ...
            <main className="flex-1 overflow-y-auto px-6 pb-6 space-y-4">
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4 text-xs text-slate-300">
                    <div className="flex items-center justify-between text-[10px] uppercase tracking-widest text-slate-400">
                        <span>Snapshot</span>
                        <span>{sensor?.timestamp ? formatTime(sensor.timestamp) : "No data"}</span>
                    </div>
                    <div className="mt-3 grid grid-cols-3 gap-3 text-center">
                        ...
                        <div>
                            ...
                            <div className="text-[10px] uppercase tracking-widest text-slate-500">CO2</div>
                        </div>
                    </div>
                ...
                {summary && (
                    <div className="rounded-2xl border border-white/10 bg-slate-900/50 p-4 text-[11px] text-slate-400">
                        <div className="text-[10px] uppercase tracking-widest text-slate-500">24h Summary</div>
                        <div className="mt-3 grid grid-cols-3 gap-3 text-center text-slate-200">
                            <div>
                                <div className="text-sm font-semibold">
                                    {summary.summary.temperature.avg?.toFixed(1) ?? "--"}C
                                </div>
                                <div className="text-[10px] uppercase tracking-widest text-slate-500">Temp avg</div>
                            </div>
                            <div>
                                <div className="text-sm font-semibold">
                                    {summary.summary.humidity.avg?.toFixed(0) ?? "--"}%
                                </div>
                                <div className="text-[10px] uppercase tracking-widest text-slate-500">Hum avg</div>
                            </div>
                            <div>
                                <div className="text-sm font-semibold">
                                    {summary.summary.co2.avg?.toFixed(0) ?? "--"}ppm
                                </div>
                                <div className="text-[10px] uppercase tracking-widest text-slate-500">CO2 avg</div>
                            </div>
                        </div>
                    </div>
                )}
                    ...
                    <div className="rounded-2xl border border-white/10 bg-slate-900/50 p-3 text-[11px] text-slate-400">
                        ...
                        <div className="space-y-2">
                            {historyItems.map((item, idx) => {
                                ...
                                const itemCo2 = toNumber(item.data?.["co2"] ?? item.data?.["gas"]);
                                return (
                                    <div key={`${item.timestamp}-${idx}`} className="flex items-center justify-between">
