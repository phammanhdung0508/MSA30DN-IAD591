...
export function RoomView() {
    ...
    useEffect(() => {
        ...
        window.localStorage.setItem("room_device_state", JSON.stringify(deviceState));
    }, [deviceState]);
    ...
    useEffect(() => {
        ...
        const loadQueue = () => {
            try {
                ...
                queue
                    .filter((item) => item && typeof item.text === "string" && typeof item.id === "number")
                    .sort((a, b) => a.id - b.id)
                    .forEach((item) => {
                ...
                window.localStorage.setItem("room_command_last_processed", String(lastProcessed));
            } catch {
                // ignore parse issues
            }
        };
        ...
        const handleEvent = (event: Event) => {
            ...
            window.localStorage.setItem("room_command_last_processed", String(incomingId));
        };

        loadQueue();
        window.addEventListener("room-command", handleEvent as EventListener);
        return () => window.removeEventListener("room-command", handleEvent as EventListener);
    }, []);
    ...
    const applyCommand = (raw: string, source: "text" | "voice" | "chat" | "device") => {
        ...
        setDeviceState((prev) => {
            const next = { ...prev };
            const targets = targetAll ? DEVICE_DEFS.map((d) => d.id) : matches.map((d) => d.id);
            targets.forEach((id) => {
                if (!action) {
                    next[id] = !prev[id];
                } else {
                            ...
                            return (
                                ...
                                >
                                    <div className="flex items-center gap-2">
                                        ...
                                        />
                                        <span className="text-xs font-semibold text-white">{device.label}</span>
                                    </div>
